...
function bind(){$$('.bottom-nav button').forEach(b=>b.onclick=()=>go(b.dataset.target));$$('[data-go]').forEach(b=>b.onclick=()=>go(b.dataset.go));$('#fuelForm').oninput=e=>{const f=new FormData(e.currentTarget),d=+f.get('distance'),l=+f.get('liters');$('#fuelPreview').textContent=d&&l?(d/l).toFixed(1)+' km/L':'-- km/L'};$('#fuelForm').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget,f=new FormData(form),liters=+f.get('liters'),capacity=tankCapacity();if(!capacity)return openTankSettings();if(liters>capacity)return toast(`給油量は${capacity}L以下で入力してください`);state.fuel.push({id:crypto.randomUUID(),createdAt:Date.now(),date:f.get('date'),distance:+f.get('distance'),liters,cost:+f.get('cost')||0,full:f.get('full')==='on',notes:f.get('notes')});await save();form.reset();form.elements.date.value=today();form.elements.full.checked=true;$('#fuelPreview').textContent='-- km/L';render();toast('給油記録を保存しました');go('home')};$('#openRecordModal').onclick=()=>$('#recordModal').showModal();$('#closeRecordModal').onclick=()=>$('#recordModal').close();$('#recordForm [name=attachment]').onchange=e=>$('#fileName').textContent=e.target.files[0]?.name||'写真またはPDFを選択';$('#recordForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);try{state.records.push({id:crypto.randomUUID(),createdAt:Date.now(),type:f.get('type'),date:f.get('date'),title:f.get('title'),odometer:+f.get('odometer')||0,cost:+f.get('cost')||0,notes:f.get('notes'),attachment:await attachment(e.currentTarget.attachment.files[0])});await save();e.currentTarget.reset();e.currentTarget.date.value=today();$('#fileName').textContent='写真またはPDFを選択';$('#recordModal').close();render();toast('記録を保存しました')}catch(err){toast(err.message)}};$('#recordFilter').onclick=e=>{const b=e.target.closest('button');if(!b)return;filter=b.dataset.filter;$$('#recordFilter button').forEach(x=>x.classList.toggle('active',x===b));records()};$('#periodControl').onclick=e=>{const b=e.target.closest('button');if(!b)return;months=+b.dataset.months;$$('#periodControl button').forEach(x=>x.classList.toggle('active',x===b));charts()};$('#recordList').onclick=async e=>{const b=e.target.closest('[data-delete]');if(!b||!confirm('この記録を削除しますか？'))return;const[k,id]=b.dataset.delete.split(':'),key=k==='fuel'?'fuel':'records';state[key]=state[key].filter(x=>x.id!==id);await save();render();toast('削除しました')};$('#exportButton').onclick=()=>{const b=new Blob([JSON.stringify(state,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=`mile-note-${today()}.json`;a.click();URL.revokeObjectURL(a.href);toast('バックアップを書き出しました')};window.onresize=()=>$('.view[data-view=analytics]').classList.contains('active')&&charts()}
